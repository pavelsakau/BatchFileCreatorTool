...
public:
	...
	wxString GetReportFilenameForPublish()
	{
		...
		switch (choice) {
		case 0:
			filename = wxString::Format("End-to-End_%s(%s).htm", destinationIP, wxT("%CURRENT_DATE% %CURRENT_TIME%"));
			break;
		case 1:
		...
		case 6:
			...
			break;
		case 7:
			filename = wxString::Format("DSCP-Loss_%s(%s).txt", destinationIP, wxT("%CURRENT_DATE% %CURRENT_TIME%"));
			break;
		}
